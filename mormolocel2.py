...
#initializare datele necesare
    ...
    def afisDrum(self, file, timp, nrNoduri = 0, nrMaximNoduri = 0, afisare_chestii = False):
        g = open(file, "a")
        l = self.obtineDrum()
        ...
        for nod in l:
            if nod.par is not None:
                g.write(str(p) + ") Broscuta a sarit de la " + str(nod.par) + " la " + str(nod) + '\n')
                g.write("Broscuta a mancat " + str(nod.insecte) + " insecte. Greutate broscuta: " + str(
                    nod.g_c) + '\n')
                p = p + 1

        g.write(str(p) + ") Broscuta a ajuns la mal in " + str(len(l)) + " sarituri.\n")

        if afisare_chestii:
            g.write('\n' + "Lungime drum: " + str(len(l)) + '\n')
            g.write("Cost drum: " +  str(len(l)) + '\n')
            g.write("Numar maxim de noduri existente la un moment dat in memorie: " + str(nrNoduri) + '\n')
            g.write("Numar total de noduri calculate: " + str(nrMaximNoduri) + '\n')
...
def a_star(gr, nrSolutiiCautate, tip_euristica):
    ...
    while len(c) > 0:
        ...
        if gr.testeaza_scop(nodCurent.x, nodCurent.y, nodCurent.g_c):
            ...
            timp = stop - start
            g = open(gr.output, "a")
            g.write("A* " + tip_euristica + ":\n\n")
            g.close()
            nodCurent.afisDrum(gr.output, timp,nod_max, nod_nr, True)
            nrSolutiiCautate -= 1
            if nrSolutiiCautate == 0:
                return
        lSuccesori = gr.genereazaSuccesori(nodCurent, tip_euristica)
        nod_nr += len(lSuccesori)
        nod_max = max(nod_max, len(lSuccesori))
        for s in lSuccesori:
            i = 0
            while i < len(c):
                if c[i].f >= s.f:
...
def uniform_cost(gr, nrSolutiiCautate=1):
    ...
    minim = float("inf")
    for s in lSuccesori:
        nrSolutiiCautate, rez = construieste_drum(gr, s, limita, nrSolutiiCautate, start, nod_max, nod_nr, tip_euristica)
        if rez == "gata":
